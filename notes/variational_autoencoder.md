...
## Architecture
VAEs learn a probabilistic representation of data $x$ within a latent space using two neural networks. The encoder takes a sample $x$ and maps it to a distribution /$q(z|x)$/ over latent variables /$z$/ . The decoder then takes these latent variables and maps them back to a distribution in the original data space /$p(x|z)$/.

## Training Objective

VAEs are trained by maximizing the Evidence Lower Bound (ELBO):

$$\mathcal{L}(\theta) = \mathbb{E}_{q(z|x)}[\log p(x|z)] - \text{KL}(q(z|x) || p(z))$$

This objective has two components. The first term, /$\mathbb{E}_{q(z|x)}[\log p(x|z)]$/, is the reconstruction loss that measures how well the decoder reconstructs the original data. The second term is the KL divergence, which acts as a regularizer by pushing the learned posterior /$q(z|x)$/ towards the prior /$p(z)$/. When we maximize the ELBO, we're simultaneously encouraging the model to accurately reconstruct the data while keeping the learned posterior close to the prior distribution.
...
where /$p(z)$/ is the prior distribution of the latent variables and /$p(x|z)$/ is the likelihood of the observed data given the latent variables.
...
The process flows like this: the one-hot encoded protein sequence passes through the encoder network, which outputs the parameters of /$q(z|x)$/ (typically the mean and variance for a Gaussian distribution) in our 2D latent space. We then sample a point /$z$/ from this distribution. The decoder takes this latent representation and reconstructs the protein sequence, outputting a probability distribution over amino acids at each position. Finally, we compute the loss by comparing the reconstructed distribution with the original sequence.
...
where $p(z)$ is the prior distribution of latent variables, $p(x|z)$ is the likelihood of observed data given the latent variables.

>[!NOTE]
> This is simply Bayes rule. 
>[!IMPORTANT]
> This is intractable as the /$p(x)=\int p(x∣z)p(z)dz$/ involves an integral over all possible latent variables $z$. In many cases, the latent space is very high-dimensional (e.g., millions of possible latent states), and the integral does not have a simple analytical solution. As a result, computing the exact likelihood requires solving this complex integral, which is often not feasible. The true posterior /$p(z∣x)$/ is also intractable to compute as it involves both the likelihood /$p(x∣z)$/ and the prior /$p(z)$/.


Variational Inference (VI) provides an elegant solution to this problem. Instead of trying to calculate the true posterior /$p(z∣x)$/ directly, we approximate it with a simpler distribution /$q(z∣x)$/ called the variational posterior (usually a simple Gaussian). The ELBO emerges from this approximation and provides a lower bound on the log-likelihood. When we maximize the ELBO, we're simultaneously minimizing the gap between the approximate posterior /$q(z∣x)$/ and the true posterior /$p(z∣x)$/ while also ensuring the model reconstructs the data well.

This approach allows us to optimize model parameters without directly computing the intractable posterior /$p(z∣x)$/ or marginal likelihood /$p(x)$/. 
